core.factory('CameraFactory', function($http){
  ...
  CameraFactory.createSquareAndUpdateStory = function(dataUrl, storyId){
    ...
    $http.put(base + '/api/stories/' + storyId + '/square', {})
    .then(function(newSquare){
      console.log('square', newSquare.data)
  ...
  var saveImage = function(dataUrl, squareId){
    ...
    .then(function(updatedSquare){
      console.log('updatedSquare', updatedSquare.data)
...
core.factory('Grafi', function(){
  ...
  Grafi.GrafiImageData = function(pixelData, width, height) {
    ...
    Grafi.convolution = function(imgData, option) {
      // check options object & set default variables
      option = option || {}
      option.monochrome = option.monochrome || false
      option.divisor = option.divisor || 1
